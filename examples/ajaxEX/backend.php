...
function get_sample_users(){
	require('config.php'); //use config.php wondering if this will work since directory is in a parent
	$conn_string = "mysql:host=$host;dbname=$database;charset=utf8mb4";
	try{
		$db = new PDO($conn_string, $username, $password); //connect to the database
		$select_query = "select id, username from `TestUsers`"; //get TestUsers database
		$stmt = $db->prepare($select_query); // stuff below is setting up
		$r = $stmt->execute();
